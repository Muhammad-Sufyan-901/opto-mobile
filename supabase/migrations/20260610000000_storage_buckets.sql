...
create policy "Owner can upload to eye-photos"
  ...
  with check (bucket_id = 'eye-photos' and auth.uid()::text = owner);

-- Owner can read (download) from eye-photos
create policy "Owner can read eye-photos"
  on storage.objects
  for select to authenticated
  using (bucket_id = 'eye-photos' and auth.uid()::text = owner);
...
create policy "Owner can upload to consultation-attachments"
  on storage.objects
  for insert to authenticated
  with check (bucket_id = 'consultation-attachments' and auth.uid()::text = owner);

-- Owner can read (download) from consultation-attachments
create policy "Owner can read consultation-attachments"
  on storage.objects
  for select to authenticated
  using (bucket_id = 'consultation-attachments' and auth.uid()::text = owner);
